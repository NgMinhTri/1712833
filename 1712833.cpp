...
wchar_t* DOCMOTTAPTIN(wchar_t *s, FILE* fp)
{
	int i = 0;
...
void doc_1_sv(FILE* fp, SINHVIEN& x)

{
	DOCMOTTAPTIN(x.MSSV, fp);

	DOCMOTTAPTIN(x.HOVATEN, fp);

	DOCMOTTAPTIN(x.khoa, fp);

	TACH(fp, x.KHOA);

	DOCMOTTAPTIN(x.ngaysinh, fp);

	DOCMOTTAPTIN(x.hinhanh, fp);

	DOCMOTTAPTIN(x.mota, fp);

	DOCMOTTAPTIN(x.sothich1, fp);
	fgetws(x.sothich2, 1000, fp);
}

int demsoluongsinhvien(FILE* fp)
{
	wchar_t s[1600];
	int dem = 0;
	while (feof(fp) == NULL) {
...
SINHVIEN* docmangSV(FILE* fp, int &n)
{
	n = demsoluongsinhvien(fp);
	fseek(fp, 0, 0);
...
void HTML_cua_1_SV(FILE* fo, sinhvien x)
{
...
void HTML_cua_mangSV(int n, sinhvien* a)
{
	for (int i = 0; i < n; i++)
	{
		FILE *fo;
